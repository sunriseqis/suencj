...
> 单文件、零依赖的 GitHub 增强油猴脚本：**界面完整汉化 + 下载一键加速 + 字体优化（云端字体·国内可用）+ 搜索引擎切换（全站生效）**，统一美化设置面板，一键开关、保存即生效。
...
| **D · 搜索引擎切换** | **任意网站**右上角引擎浮层 | 百度 / Bing / Google / DuckDuckGo / 搜狗 一键搜索 |

> 脚本 `@match` 为全站匹配（以支撑模块 D 在百度/Google 等搜索页使用）。**汉化与字体优化带 GitHub 域名守卫，只在 GitHub 系列站点生效**，不会污染其他网站；下载加速只匹配 GitHub 下载地址。

## 📦 安装

1. 安装浏览器扩展 **Tampermonkey**（油猴）。
2. 打开 Tampermonkey → **添加新脚本** → 清空编辑框 → 粘贴 `suencj.user.js` 全文 → 保存（Ctrl+S）。
3. 访问任意网站即可生效：GitHub 页享受全部四大功能；其他网站右上角出现搜索引擎切换浮层。页面**右下角 ⚙ 齿轮按钮**打开统一设置面板（或油猴菜单「GitHub 增强套件 · 打开设置」）。
...
### D · 搜索引擎切换（全站生效）
- **任意网站**页面右上角「用：百度 Bing Google DuckDuckGo 搜狗」浮层（百度/Google/搜狗搜索结果页、GitHub、普通站点均可）。
- 取词优先级：当前 URL 搜索参数（`q` / `wd`（百度）/ `query`（搜狗）/ `search` / `keyword` 等）→ GitHub 仓库页 `owner/repo` → 当前页搜索框输入。
- 点击后新标签页打开对应引擎搜索。
...
- `node --check` 语法校验通过。
- jsdom 全量测试 PASS：
  - **全站生效**：百度搜索页显示浮层、取 `wd` 参数正确打开 Google/百度；普通站点浮层仍显示、无搜索词不误开；GitHub 仓库页取 `owner/repo`。
  - **域名守卫**：非 GitHub 站点不触发汉化、不注入字体；GitHub 页汉化/字体/加速图标全部正常。
  - 既有回归：搜索框保护、加速图标幂等注入与不误注入、设置面板保存。

## ⚠️ 已知限制

- 词库来自社区，极少数最新上线的 UI 字符串可能仍为英文（社区更新后自动生效，也可在脚本内 `SUPPLEMENT_STATIC` 补充）。
- 汉化 / 字体优化仅匹配 `github.com` / `gist.github.com` / `skills` / `education` / `githubstatus` 系列域名；搜索引擎切换为全站生效。
